...
export default class PluginCreate {
    ...
    public async run(noPrompt = false): Promise<void> {
        ...
        const packageInfo = await fs.readJSON(path.resolve(this.targetDir, "package.json"));
        ...
        await fs.writeJSON(path.resolve(this.targetDir, "package.json"), packageInfo, {spaces: "\t"});
